//
//  FrameTVTopShelf.swift
//  FrameTVTopShelf (tvOS Top Shelf extension target)
//
//  Shows Continue Watching and Recently Added rows on the Apple TV home screen, above
//  the FrameTV icon. Reads the same App Group snapshot the app writes for widgets, so
//  no extra app-side work is needed. Selecting an item opens FrameTV via a frametv://
//  deep link.
//
//  IMPORTANT — Xcode setup (see TOPSHELF_SETUP.md):
//   • This file belongs to the FrameTVTopShelf extension target only (tvOS).
