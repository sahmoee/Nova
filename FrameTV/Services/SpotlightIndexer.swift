...
//  shows from the home screen. Tapping a result opens FrameTV via a frametv:// deep
//  link to that title.
//
...
enum SpotlightIndexer {
    /// Domain used so we can clear just FrameTV's items if needed.
    private static let domain = "com.frametv.library"
    ...
    /// Removes everything FrameTV put in Spotlight (e.g. on library clear).
    static func clear() {
    ...
    private static func makeItem(_ item: MediaItem) -> CSSearchableItem? {
        ...
        let identifier = "frametv://\(isShow ? "show" : "movie")/\(contentID.stableKey)"
    ...
    /// Pulls the frametv:// deep-link URL out of a Spotlight result's activity.
