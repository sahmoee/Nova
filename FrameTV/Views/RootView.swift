...
struct RootView: View {
    ...
    #else
    /// iOS / iPadOS: iPad gets a dedicated sidebar layout; iPhone keeps the
    /// standard bottom tab bar or the floating pill.
    @ViewBuilder
    private var rootContent: some View {
        if Theme.isPad {
            iPadSidebarRoot
    ...
    /// the App Store on iPad) with the selected section in the detail column. This
    /// uses the large canvas properly instead of an iPhone-style bottom bar.
    @ViewBuilder
