...
struct AddAddonView: View {
    ...
    private var normalizedURL: URL? {
        ...
        // id/token from their configure page, not a full URL. If the field holds a
        // bare token and this preset has a known host, expand it into a real URL so
        // the install doesn't hang trying to resolve the token as a hostname.
        ...
        if looksBare, let host = preset?.hostForBareConfig {
            ...
            // Strip a stremio:// style scheme if present.
    ...
    private func install(_ explicitURL: URL? = nil) {
        ...
        Task {
            do {
                ...
                // stick on "Installing…" if the network stalls beyond the request
                // timeout (e.g. a token typed as a host that never resolves).
