...
if not ls:
    print("LibraryStore anchor not found; nothing to do."); sys.exit(0)
fileref_ls = ls.group(1)
psv = re.search(r"([0-9A-F]{24}) /\* PlayerSettingsView\.swift \*/ = \{isa = PBXFileReference", s)
psv_ref = psv.group(1) if psv else fileref_ls
bf_guids = re.findall(r"([0-9A-F]{24}) /\* LibraryStore\.swift in Sources \*/,", s)
changed = False
for fname, group in [("LibraryFolderStore.swift","Services"),("LibraryFoldersView.swift","Settings")]:
    ...
    sib = f'\t\t\t\t{fileref_ls} /* LibraryStore.swift */,\n' if group=="Services" else f'\t\t\t\t{psv_ref} /* PlayerSettingsView.swift */,\n'
    if sib in s:
        s = s.replace(sib, sib + f'\t\t\t\t{fref} /* {fname} */,\n', 1)
...
if changed:
    open(pbx, "w").write(s)
    print("Registered LibraryFolder files in project.pbxproj")
else:
    print("LibraryFolder files already registered")
