//
//  NovaTopShelf.swift
//  NovaTopShelf (tvOS Top Shelf extension target)
//
//  Shows Continue Watching and Recently Added rows on the Apple TV home screen, above
//  the Nova icon. Reads the same App Group snapshot the app writes for widgets, so
//  no extra app-side work is needed. Selecting an item opens Nova via a nova://
//  deep link.
//
//  IMPORTANT — Xcode setup (see TOPSHELF_SETUP.md):
//   • This file belongs to the NovaTopShelf extension target only (tvOS).
//   • WidgetShared.swift must be a member of this target too (it holds the snapshot model).
//   • This target and the tvOS app must both enable the retained App Group
//     "group.astra.ios" so existing Astra data remains accessible.
//
