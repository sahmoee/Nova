...
extension Date {
    ...
    var mediumDateText: String { NovaDateFormatters.mediumDate.string(from: self) }

    /// Medium date with short time — e.g. "Jan 5, 2026 at 3:30 PM". Used for
    /// "last backed up" / snapshot timestamps.
    var mediumDateTimeText: String { NovaDateFormatters.mediumDateTime.string(from: self) }
}

/// Cached formatter instances shared across the app.
enum NovaDateFormatters {
