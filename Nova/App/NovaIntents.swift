...
//  on Nova" or "Open Nova Library", and add Nova actions to the Shortcuts app.
//
...
enum PendingRoute {
    ...
    /// Store a nova:// URL string to be routed on next activation.
...
/// "Continue watching on Nova" — opens the app to the Library / Continue Watching.
struct ContinueWatchingIntent: AppIntent {
    static let title: LocalizedStringResource = "Continue Watching"
    static let description = IntentDescription("Open Nova to your Continue Watching list.")
    static let openAppWhenRun = true

    @MainActor
    func perform() async throws -> some IntentResult {
        PendingRoute.set("nova://continue")
        return .result()
    }
}

/// "Open Nova Library" — jumps straight to the Library tab.
struct OpenLibraryIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Library"
    static let description = IntentDescription("Open Nova to your library.")
    static let openAppWhenRun = true

    @MainActor
    func perform() async throws -> some IntentResult {
        PendingRoute.set("nova://library")
        return .result()
    }
}

/// "Discover on Nova" — opens the Discover tab.
struct OpenDiscoverIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Discover"
    static let description = IntentDescription("Open Nova to Discover.")
    static let openAppWhenRun = true

    @MainActor
    func perform() async throws -> some IntentResult {
        PendingRoute.set("nova://discover")
        return .result()
    }
}
