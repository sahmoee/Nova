...
struct MediaGridDetailView: View {
    ...
    // Needed to re-inject into UIHostingConfiguration cells on iOS, which do not
    // inherit environment objects from the surrounding SwiftUI hierarchy.
    @EnvironmentObject private var library: LibraryStore
    ...
    var body: some View {
        ZStack {
            ...
            if items.isEmpty {
                ...
                #if os(iOS)
                // Primary-scroll grid: UICollectionView reuse + poster prefetch for
                // large collections. Cards are the same SwiftUI views, hosted per
                // cell; `library` is re-injected because hosted cells don't inherit it.
                PosterCollectionGrid(
                    items: items,
                    spacing: Theme.Spacing.lg,
                    sectionInsets: EdgeInsets(top: Theme.Spacing.edge,
