...
//  auto-select picks the best), Nova resolves it to a playable URL, and hands off
...
struct StreamPickerView: View {
    ...
    @ViewBuilder
    private var addonProgressPanel: some View {
        if !addonProgress.isEmpty {
            VStack(alignment: .leading, spacing: 8) {
                ...
                ForEach(addonProgress.values.sorted(by: { $0.addonName < $1.addonName }), id: \.addonID) { progress in
                    HStack(spacing: 8) {
                        ...
                        Text(progress.addonName)
                            .font(.appFont(15, weight: .semibold))
                            .foregroundStyle(Theme.Colors.textPrimary)
                        Spacer()
                        if progress.phase == .succeeded {
                            Text("\(progress.streams.count) · \(progress.latencyMS ?? 0) ms")
    ...
    private func progressIcon(_ phase: AddonStreamProgress.Phase) -> String {
        switch phase {
        case .started: return "hourglass"
        case .succeeded: return "checkmark.circle.fill"
        case .failed: return "exclamationmark.triangle.fill"
        case .circuitOpen: return "pause.circle.fill"
        }
    }

    private func progressColor(_ phase: AddonStreamProgress.Phase) -> Color {
        switch phase {
