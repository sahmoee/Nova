...
//  AIOStreams and Comet, enable/disable, and remove. Nova ships no addons itself;
...
struct AddonsView: View {
    ...
    @State private var editingAddon: InstalledAddon?
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: Theme.Spacing.lg) {
                ...
                if store.addons.isEmpty {
                    ...
                    ForEach(filteredAddons) { addon in
                        reorderable(addonRow(addon), addon: addon)
                            .contextMenu {
                                Button { store.move(id: addon.id, offset: -1) } label: {
                                    Label("Move Earlier", systemImage: "arrow.up")
                                }
                                Button { store.move(id: addon.id, offset: 1) } label: {
                                    Label("Move Later", systemImage: "arrow.down")
                                }
                                if let catalog = addon.catalogs.first {
                                    NavigationLink {
                                        AddonCatalogBrowserView(addon: addon, catalog: catalog)
                                    } label: {
                                        Label("Browse Catalog", systemImage: "rectangle.stack")
                                    }
                                }
                                Button {
                                    testAddon(addon)
                                ...
                                #if os(iOS)
                                if !addon.requiresConfiguredURL {
                                    Button {
                                        qrAddon = addon
                                    } label: {
        ...
        .confirmationDialog(
            pendingRemoval.map { "Remove \($0.name)?" } ?? "Remove Addon?",
            isPresented: Binding(
            ...
            Button("Remove Addon", role: .destructive) {
                if let addon = pendingRemoval { store.remove(addon) }
                pendingRemoval = nil
            }
            Button("Cancel", role: .cancel) { pendingRemoval = nil }
        } message: {
            Text("You can undo immediately after removal.")
        }
        .safeAreaInset(edge: .bottom) {
            if let removed = store.lastRemoved {
                HStack {
                    ...
                    Spacer()
                    Button("Undo") { store.undoLastRemoval() }
        ...
        .fileExporter(isPresented: $showExporter,
                      document: exportDoc,
                      contentType: .json,
                      defaultFilename: "Nova-Addons") { _ in }
    ...
    @ViewBuilder
    private func reorderable<Content: View>(_ content: Content, addon: InstalledAddon) -> some View {
        #if os(iOS)
        content
            .draggable(addon.id.uuidString)
            .dropDestination(for: String.self) { ids, _ in
                guard let text = ids.first, let sourceID = UUID(uuidString: text) else { return false }
                store.move(id: sourceID, before: addon.id)
                return true
            }
        #else
        content
    ...
    private var filteredAddons: [InstalledAddon] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return store.addons.filter { addon in
            if let selectedCategory, addon.category != selectedCategory { return false }
            guard !query.isEmpty else { return true }
            return addon.name.lowercased().contains(query)
                || (addon.description?.lowercased().contains(query) == true)
                || (addon.category?.lowercased().contains(query) == true)
                || addon.tags.contains(where: { $0.lowercased().contains(query) })
        }
    }
    ...
    private var filterBar: some View {
        HStack(spacing: Theme.Spacing.sm) {
            ...
            TextField("Search addons, categories, or tags", text: $searchText)
                .font(.appFont(17))
                #if os(iOS)
    ...
    private func testAddon(_ addon: InstalledAddon) {
        Task {
            let start = Date()
            do {
                _ = try await env.addonClient.probe(addon: store.resolved(addon))
