...
//  URL; Nova then queries it for streams (and optionally subtitles/catalogs).
...
struct InstalledAddon: Identifiable, Codable, Hashable {
    ...
    /// The complete URL is stored in Keychain; this model only persists a redacted URL.
    var requiresConfiguredURL: Bool {
        let host = manifestURL.host?.lowercased() ?? ""
        return host.contains("aiostreams") || host.contains("comet")
    }

    /// Keychain account used for the complete transport URL, stable across iCloud
    /// metadata sync because the add-on UUID is synced with the redacted record.
    var secureManifestAccount: String { "addon.manifest.\(id.uuidString)" }

    /// True when the URL contains credentials, sensitive query parameters, or a
    /// configured path that must not be copied into ordinary persistence.
    var manifestURLContainsSensitiveConfiguration: Bool {
        guard let components = URLComponents(url: manifestURL, resolvingAgainstBaseURL: false) else {
            return requiresConfiguredURL
        }
        if components.user != nil || components.password != nil { return true }
        let sensitiveNames = ["token", "key", "apikey", "api_key", "access_token", "auth", "secret"]
        if components.queryItems?.contains(where: { sensitiveNames.contains($0.name.lowercased()) }) == true {
            return true
        }
        return requiresConfiguredURL && components.path != "/manifest.json"
    }

    /// A safe endpoint suitable for JSON, iCloud, logs, exports, and display. It
    /// intentionally cannot recreate a configured add-on without Keychain access.
    var redactedManifestURL: URL {
        ...
        let sensitiveNames = ["token", "key", "apikey", "api_key", "access_token", "auth", "secret"]
        components.queryItems = components.queryItems?.filter { !sensitiveNames.contains($0.name.lowercased()) }
        if requiresConfiguredURL { components.path = "/manifest.json" }
...
/// A raw stream entry from an addon, before Nova parses/ranks it.
