...
actor TMDBClient {
    ...
    func hydrateSeries(_ item: CatalogItem,
                       onPartial: (@Sendable (CatalogItem) -> Void)? = nil) async throws -> CatalogItem {
        var result = item
        ...
        let prioritySeason = seasonNumbers.first(where: { $0 > 0 }) ?? seasonNumbers.first
        if let prioritySeason, let season = await loadSeason(tmdbID: tmdb, number: prioritySeason) {
            seasons.append(season)
            result.seasons = seasons
            onPartial?(result)
        }

        let remaining = seasonNumbers.filter { $0 != prioritySeason }
        await withTaskGroup(of: SeasonInfo?.self) { group in
            var iterator = remaining.makeIterator()
            for _ in 0..<min(3, remaining.count) {
                if let number = iterator.next() {
                    group.addTask { [self] in await loadSeason(tmdbID: tmdb, number: number) }
                }
            }
    ...
    private func loadSeason(tmdbID: Int, number: Int) async -> SeasonInfo? {
        guard let detail: TMDBSeasonDetail = try? await get("tv/\(tmdbID)/season/\(number)") else {
            return nil
        }
        let episodes = (detail.episodes ?? []).map { ep in
            EpisodeInfo(
                season: ep.seasonNumber ?? number,
        ...
        return SeasonInfo(number: number, name: detail.name, episodes: episodes)
    }
    ...
    private func get<T: Decodable>(_ path: String, query: [String: String] = [:]) async throws -> T {
        ...
        do {
            ...
            NovaLog.network.error("TMDB request failed: \(error.localizedDescription, privacy: .public)")
    ...
    // FIX: cache the air-date formatter. It was being re-created for every episode
    // of every season during series hydration (a hot path); DateFormatter creation
    // is expensive, and DateFormatter is documented thread-safe on modern OSes.
    // DateFormatter isn't `Sendable`; this one is fully configured in its initializer
    // and only ever read, so opt out of the concurrency check explicitly.
