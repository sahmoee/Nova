...
actor RealSMBProvider: SMBProviding {
    ...
    func connect(to share: SMBShare) async throws {
        ...
        do {
            ...
            NovaLog.network.error("SMB connect failed for user length \(user.count, privacy: .public), password length \(password.count, privacy: .public): \(String(describing: error), privacy: .public)")
            throw mapError(error)
        }

        client = manager
        connectedShare = share
        NovaLog.network.info("Connected to SMB share \(share.shareName, privacy: .public)")
    }
