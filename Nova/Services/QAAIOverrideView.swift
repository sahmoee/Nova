...
struct QAAIOverrideView: View {
    ...
    private func request(_ method: String, _ body: Data? = nil) async throws -> Data { var c = URLComponents(string: "https://api.sowensstudios.com/_unified/qa/ai-config")!; if method == "GET" { c.queryItems = [.init(name: "app", value: app)] }; var r = URLRequest(url: c.url!); r.httpMethod = method; r.setValue("6352", forHTTPHeaderField: "X-QA-Passcode"); if let body { r.httpBody = body; r.setValue("application/json", forHTTPHeaderField: "Content-Type") }; let (d, p) = try await URLSession.shared.data(for: r); guard (p as? HTTPURLResponse)?.statusCode == 200 else { throw URLError(.badServerResponse) }; return d }
