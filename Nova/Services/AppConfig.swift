...
//    2. A bundled/Documents `NovaConfig.json` provides fallback defaults.
//
...
/// Keys for the various credentials Nova can use.
...
/// Shape of the optional NovaConfig.json fallback file.
struct NovaConfigFile: Codable {
...
final class AppConfig: @unchecked Sendable {
    ...
    /// Loaded once at init from the bundled/Documents NovaConfig.json. Immutable
    /// after construction, which keeps the shared singleton concurrency-safe.
    let fileConfig: NovaConfigFile?
    ...
    /// Looks for NovaConfig.json first in the app bundle, then in the app's
    /// Documents directory (so it can be dropped in via file sharing).
    private static func loadFileConfig() -> NovaConfigFile? {
        let decoder = JSONDecoder()

        if let bundleURL = Bundle.main.url(forResource: "NovaConfig", withExtension: "json"),
           let data = try? Data(contentsOf: bundleURL),
           let cfg = try? decoder.decode(NovaConfigFile.self, from: data) {
            return cfg
        }
        ...
        if let docs {
            let docURL = docs.appendingPathComponent("NovaConfig.json")
    ...
    /// NOVA_SHARED_TOKEN secret is configured.
