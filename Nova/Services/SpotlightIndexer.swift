...
//  shows from the home screen. Tapping a result opens Nova via a nova:// deep
//  link to that title.
//
...
enum SpotlightIndexer {
    /// Domain used so we can clear just Nova's items if needed.
    private static let domain = "com.nova.library"
    ...
    /// Removes everything Nova put in Spotlight (e.g. on library clear).
    static func clear() {
    ...
    private static func makeItem(_ item: MediaItem) -> CSSearchableItem? {
        ...
        let identifier = "nova://\(isShow ? "show" : "movie")/\(contentID.stableKey)"
    ...
    /// Pulls the nova:// deep-link URL out of a Spotlight result's activity.
