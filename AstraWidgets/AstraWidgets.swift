//
//  AstraWidgets.swift
//  AstraWidgets (widget extension target)
//
...
struct AstraProvider: TimelineProvider {
    func placeholder(in context: Context) -> AstraTimelineEntry {
        AstraTimelineEntry(date: Date(), snapshot: .empty, posters: [:])
    }
...
struct ContinueWatchingWidget: Widget {
    let kind = "AstraContinueWatching"
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: AstraProvider()) { entry in
